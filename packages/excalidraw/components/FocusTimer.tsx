...
export const FocusTimer = ({ onExpired }: FocusTimerProps) => {
  ...
  const customMinutesParsed = parseCustomMinutes(customMinutes);
  const customMinutesInvalid =
    customMinutes.trim().length > 0 && customMinutesParsed == null;
  const canStart =
    status === "idle" && durationMs > 0 && !customMinutesInvalid;
            ...
            {showPresets && (
              <>
                ...
                <div className="FocusTimer__custom">
                  <label
                    className="FocusTimer__custom-label"
